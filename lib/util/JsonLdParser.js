...
class JsonLdParser
{
    ...
    {
        let result = {
            subject: JsonLdParser.fixUri(triple.subject.value),
            predicate: JsonLdParser.fixUri(triple.predicate.value),
            object: JsonLdParser.convertEntity(triple.object),
            graph: triple.graph ? JsonLdParser.fixUri(triple.graph.value) : undefined
        };
    ...
    static convertEntity (entity)
    {
        ...
        if (entity.type !== 'literal')
            return JsonLdParser.fixUri(entity.value);
    ...
    // jsonld library doesn't encode URIs
    static fixUri (uri)
    {
        // don't do this for all URIs since most of them will already be encoded!
        if (uri.match(/[\x00-\x20<>"{}|^`\\]/))
