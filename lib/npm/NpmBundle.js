...
class NpmBundle extends Bundle
{
    ...
    getUri ()
    {
        if (this.name[0] === '@')
        {
            let slash = this.name.indexOf('/');
            if (slash >= 0) {
                return this.getBaseUri() + '@' + encodeURIComponent(this.name.substring(1, slash)) + '/' + encodeURIComponent(this.name.substring(slash + 1));
            }
        }
    ...
    getUserMap ()
    {
        ...
        function handleUser (user, map, isNpmName)
        {
            ...
            if (!map[mail])
                map[mail] = {};
            map[mail].email = `mailto:${encodeURI(mail)}`;
            if (user.url)
    ...
    getJsonLd (output)
    {
        return NpmContext.addContext(this, output).then(json =>
        {
            ...
            if (json.name)
                json['owl:sameAs'] = 'https://www.npmjs.com/package/' + encodeURIComponent(json.name);
            ...
            if (json.time)
            {
                if (json.time.created)
                    json.created = json.time.created;
                if (json.time.modified)
                    json.modified = json.time.modified;
            }
