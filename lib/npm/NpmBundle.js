...
class NpmBundle extends Bundle
{
    ...
    getJsonLd (output)
    {
        return NpmContext.addContext(this).then(json =>
        {
            ...
            // TODO: UGLY!
            let ignore = ['owl:sameAs', 'spdx:licenseInfoFromFiles', 'dcterms:license'];
            ...
            if (output)
                json.readme = { '@id': this.getUri() + '/README', 'rdfs:label': json.readme };
            else
                json.readme = this.getUri() + '/README';
            ...
            NpmContext.updateUnusedValues(json['@graph'][0], ignore);
            return json;
        });
    }
}
