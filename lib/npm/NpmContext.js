    ...
    static addContext (thingy)
    {
        return Promise.all([thingy.getJson(), thingy.getUserMap()]).then(([json, userMap]) =>
        {
            ...
            if (json.license)
            {
                ...
                {
                    let id = 'file://' + json.license.substring('SEE LICENSE IN'.length);
                    json['spdx:licenseInfoFromFiles'] = { '@id': id, 'spdx:name': json.license };
                    json['spdx:name'] = json.license;
                    
                    json['dcterms:license'] = { '@id': license, 'rdfs:label': json.license };
                    delete json.license;
                }
                else
                {
