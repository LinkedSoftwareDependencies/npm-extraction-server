...
let rootContext = JSON.parse(fs.readFileSync(path.join(__dirname, '../contexts/npm.jsonld')))['@context'];
...
class NpmContext
{
    static mergeContexts (contexts)
    {
        ...
        {
            if (context === 'https://linkedsoftwaredependencies.org/contexts/npm.jsonld')
                context = rootContext;
        ...
        for (let entry of jsonld)
        {
            ...
            for (let key in entry)
                if (!NpmContext.isMapped(key, context))
                    keys.push(NpmContext.recursiveUpdateUnusedValues(key, entry[key], entry['@id'] + '/' + key));
            if (keys.length === 0)
                return;
            if (keys.length === 1)
                keys = keys[0];
    ...
    static recursiveUpdateUnusedValues (key, value, id)
    {
        let result = { '@id': id };
        if (key)
        ...
        if (_.isArray(value))
        {
            value = value.map((v, i) => NpmContext.recursiveUpdateUnusedValues(null, v, id + '/' + i));
        ...
        else if (_.isObject(value))
        {
            ...
            for (let subKey in value)
                subKeys.push(NpmContext.recursiveUpdateUnusedValues(subKey, value[subKey], id + '/' + subKey));
            if (subKeys.length === 1)
                subKeys = subKeys[0];
            result['npm:value'] = subKeys;
        }
        else if (key)
    ...
    static handleRepository (url)
    {
        ...
        if (type === 'git')
            url = 'https://github.com/' + encodeURIComponent(url);
        else if (type === 'gist')
            url = 'https://gist.github.com/' + encodeURIComponent(url.substring('gist:'.length));
        else if (type === 'bitbucket')
            url = 'https://bitbucket.org/' + encodeURIComponent(url.substring('bitbucket:'.length));
        else if (type === 'gitlab')
            url = 'https://gitlab.org/' + encodeURIComponent(url.substring('gitlab:'.length));
    ...
    static addContext (thingy, output)
    {
        return Promise.all([thingy.getJson(), thingy.getUserMap()]).then(([json, userMap]) =>
        {
            ...
            json['@context'] = ['https://linkedsoftwaredependencies.org/contexts/npm.jsonld'];
            ...
            if (json.license)
            {
                ...
                else
                {
                    function parseSpdx (license)
                    {
                        let result = {};
                        if (license.license)
                            result['@id'] = 'https://spdx.org/licenses/' + encodeURIComponent(license.license) + '.html';
                        if (license.exception)
                            result['spdx:licenseException'] = 'https://spdx.org/licenses/' + encodeURIComponent(license.exception) + '.html';
            ...
            if (json.repository && json.repository.url)
            {
                ...
                if (url.indexOf('://') < 0 && !validUrl.isUri(url) && validUrl.isUri('ssh://' + url))
                    url = 'ssh://' + url;
            ...
            function handlePerson (person, nameIsId)
            {
                ...
                else if (person.email && !person['@id'])
