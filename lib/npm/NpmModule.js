...
class NpmModule extends Module
{
    ...
    getTarball ()
    {
        if (this.tarballPromise)
            return this.tarballPromise;

        return this.getJson().then(json =>
        {
            if (!json.dist || !json.dist.tarball)
                throw new Error("No tarball found");
            this.tarballPromise = Tarball.fromUrl(json.dist.tarball);
            return this.tarballPromise;
        })
    }
    ...
    getJsonLd (output)
    {
        return NpmContext.addContext(this).then(json =>
        {
            ...
            if (json['lsd:components'])
            {
                ...
                {
                    // don't download tarballs if doing output
                    if (output)
            ...
            function contextsFallback(e)
            {
                let contexts = {};
                for (let uri in json['lsd:contexts'])
                    contexts[uri] = { 'rdfs:label': json['lsd:contexts'][uri] };
                json['lsd:contexts'] = contexts;
            }

            if (json['lsd:contexts'])
            {
                // don't download tarballs if doing output
                if (!json.dist || !json.dist.tarball || output)
                    contextsFallback();
                else {
                    let promise = this.getTarball()
                        .then(data => handleContexts(json['lsd:contexts'], data))
                        .then(data => json['lsd:contexts'] = data)
                        .catch(contextsFallback);
                    promises.push(promise);
                }
            }
...
function handleComponent (path, data) {
    ...
    while (paths.length > 0) {
        ...
        let result = Tarball.resolvePath(path, data);
        handled[path] = true;
...
function handleContexts (contexts, data)
{
    ...
    for (let uri in contexts)
        result[uri] = JSON.parse(Tarball.resolvePath(contexts[uri], data));
    return result;
}
